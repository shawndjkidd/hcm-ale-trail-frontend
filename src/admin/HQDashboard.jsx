...
export default function HQDashboard() {
  ...
  const loadData = async (from, to) => {
    ...
    if (overviewResult.ok) {
      ...
      setError(overviewResult.error || 'Failed to load data');
    }
  ...
  const handleExport = async (format) => {
    ...
    if (result.ok) {
      if (format === 'json') {
        ...
        alert('CSV export: ' + result.count + ' participants');
      }
    } else {
  ...
  return (
    <div className="admin-content">
      ...
      {activeTab === 'events' && (
        <div className="admin-card">
          <h3 className="admin-card-title">All Events</h3>
          {events.length === 0 ? (
              ...
              <tbody>
                {leaderboard.map((entry) => (
                  <tr key={entry.participantId}>
                    <td>
                      <span className={`admin-rank ${entry.rank === 1 ? 'gold' : entry.rank === 2 ? 'silver' : entry.rank === 3 ? 'bronze' : 'default'}`}>
                        {entry.rank}
                      </span>
                    </td>
                    <td><strong>{entry.displayName || entry.name || '--'}</strong></td>
                    <td>
                      <a href={`mailto:${entry.email}`} style={{ color: 'var(--admin-primary)' }}>
                        {entry.email}
                      </a>
                    </td>
                    <td>{formatTime(entry.completionTimeMs)}</td>
                    <td style={{ color: 'var(--admin-text-muted)' }}>
                      {new Date(entry.completedAt).toLocaleDateString('en', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric'
                      })}
      ...
      {activeTab === 'export' && (
        <div className="admin-card">
          ...
          <p style={{ color: 'var(--admin-text-muted)', marginBottom: 20 }}>
            Download a list of all participants with their email, name, country, and progress.
          </p>
