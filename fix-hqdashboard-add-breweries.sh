...
FILE="src/admin/HQDashboard.jsx"
BACKUP="src/admin/HQDashboard.backup.$(date +%s).jsx"
...
echo "==> Commit + push..."
git add "$FILE" "$BACKUP" 2>/dev/null || true
git add "$FILE"
git commit -m "Admin UI: HQDashboard adds Breweries tab + keeps Overview + Events" || true
git push

echo "✅ Done. If anything looks off, your backup is: $BACKUP"
